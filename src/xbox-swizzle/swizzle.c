...
/*
 * Create masks representing the interleaving of each linear texture dimension
 * (x, y, z). These can be used to map linear texture coordinates to a swizzled
 * "Z" offset. For example, a 2D 8x32 texture needs 3 bits for x, and 5 bits for
 * y: mask_x:  00010101 mask_y:  11101010 mask_z:  00000000 for "Z": yyyxyxyx
 */
...
static inline void swizzle_box_internal(const uint8_t *src_buf,
                                        unsigned int width, unsigned int height,
                                        unsigned int depth, uint8_t *dst_buf,
...
static inline void
unswizzle_box_internal(const uint8_t *src_buf, unsigned int width,
    ...
    for (z = 0; z < depth; z++) {
        ...
        for (y = 0; y < height; y++) {
            ...
            const uint8_t *src_tmp =
                src_buf + (off_y + off_z) * bytes_per_pixel;
